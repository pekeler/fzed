...
use gpui::{
    ...
    Focusable, SharedString, Subscription, Task, TaskExt, WeakEntity, Window,
};
...
pub fn init(cx: &mut App) {
    ...
    cx.observe_new(|workspace: &mut Workspace, _, cx| {
        ...
        workspace.register_action(|workspace, _: &git::fossil_actions::Timeline, window, cx| {
            if workspace_active_repository_kind(workspace, cx).is_fossil() {
                window.dispatch_action(git_panel::ToggleFocus.boxed_clone(), cx);
                window.dispatch_action(git_panel::ActivateHistoryTab.boxed_clone(), cx);
            }
        });
        workspace.register_action(
...
fn render_remote_button(
    ...
    match upstream {
        Some(Upstream {
            ...
            (0, 0) if show_fetch_button => Some(
                remote_button::render_fetch_button(keybinding_target, id, in_progress_operation)
                    .into_any_element(),
            ),
            (0, 0) => None,
            (ahead, 0) => Some(
                remote_button::render_push_button(
                    keybinding_target,
                    id,
                    ahead,
            ...
            (ahead, behind) => Some(
                remote_button::render_pull_button(
                    keybinding_target,
                    id,
                    ahead,
                    behind,
        ...
        Some(Upstream {
            ...
            remote_button::render_republish_button(keybinding_target, id, in_progress_operation)
                .into_any_element(),
        ),
        None => Some(
            remote_button::render_publish_button(keybinding_target, id, in_progress_operation)
                .into_any_element(),
        ),
    }
}
...
mod remote_button {
    ...
    use ui::{
        App, Button, ButtonCommon, ButtonSize, ButtonStyle, Clickable, Color, CommonAnimationExt,
        ContextMenu, Disableable, ElementId, FluentBuilder, Icon, IconName, IconSize, IntoElement,
        Label, LabelCommon, LabelSize, LineHeightStyle, ParentElement, PopoverMenu, SharedString,
