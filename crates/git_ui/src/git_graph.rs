...
fn view_revision_label(repository_kind: RepositoryKind) -> &'static str {
    if repository_kind.is_fossil() {
        "View Check-in"
    } else {
        "View Commit"
...
fn copy_revision_id_label(repository_kind: RepositoryKind) -> &'static str {
    if repository_kind.is_fossil() {
        "Copy Hash"
    } else {
        "Copy SHA"
...
impl GitGraph {
    ...
    fn repository_kind(&self, cx: &App) -> RepositoryKind {
    ...
    fn deploy_entry_context_menu(
        ...
        let git_tasks = if repository_kind.is_fossil() {
            Vec::new()
        } else {
            self.git_task_context(sha, ref_name.as_deref(), cx)
        ...
        let header = match &ref_name {
            ...
            None => format!("{} {sha_short}", revision_label(repository_kind)),
        };
        ...
        let context_menu = ContextMenu::build(window, cx, |context_menu, window, _| {
            let context_menu = context_menu
                ...
                .header(header)
                .entry(
                    view_revision_label(repository_kind),
                ...
                .entry(
                    copy_revision_id_label(repository_kind),
        ...
        v_flex()
            ...
            .child(
                h_flex().p_1p5().w_full().child(
                    Button::new("view-commit", view_revision_label(self.repository_kind(cx)))
                        .full_width()
...
mod tests {
    ...
    fn graph_tab_title_tracks_repository_kind() {
        ...
        assert_eq!(view_revision_label(RepositoryKind::Git), "View Commit");
