...
use git::{Oid, repository::RepositoryKind};
...
pub(crate) fn commit_context_menu(
    ...
    let repository_kind = repository
        ...
        .unwrap_or(RepositoryKind::Git);
    let git_tasks = if repository_kind.is_fossil() {
        Vec::new()
    } else {
        git_context_menu_tasks(
    ...
    ContextMenu::build(window, cx, move |context_menu, _, _| {
        context_menu
            ...
            .when(source == CommitContextMenuSource::GitPanel, |menu| {
                let label = if repository_kind.is_fossil() {
                    "Show in Timeline"
                } else {
                    "Show in Git Graph"
                };
            ...
            .when(!repository_kind.is_fossil(), |menu| {
                menu.map(|mut menu| {
...
pub(crate) fn copy_revision_id_label(repository_kind: RepositoryKind) -> &'static str {
    if repository_kind.is_fossil() {
        "Copy Hash"
    } else {
        "Copy SHA"
