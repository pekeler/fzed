...
impl BranchList {
    ...
    pub(crate) fn set_branch_filter(
        ...
        if !supports_branch_filter(self.picker.read(cx).delegate.repo.as_ref(), cx) {
            return;
        }
...
fn supports_branch_filter(repository: Option<&Entity<Repository>>, cx: &App) -> bool {
    repository.is_none_or(|repository| !repository.read(cx).kind().is_fossil())
}
...
impl BranchListDelegate {
    fn new(
        ...
        let branch_filter = if supports_branch_filter(repo.as_ref(), cx) {
            cx.try_global::<GlobalBranchFilter>()
...
impl PickerDelegate for BranchListDelegate {
                ...
                .child(
                    h_flex()
                        ...
                        .map(|this| {
                            ...
                            this.gap_1().justify_between().when(
                                supports_branch_filter(self.repo.as_ref(), cx),
                                |this| {
                                    this.child(
    ...
    fn render_match(
        ...
        let section_header = (supports_branch_filter(self.repo.as_ref(), cx)
            && self.branch_filter == BranchFilter::All)
...
mod tests {
    ...
    #[gpui::test]
    async fn test_branch_filter_is_disabled_for_fossil(cx: &mut TestAppContext) {
        init_test(cx);
        cx.update(|cx| cx.set_global(GlobalBranchFilter(BranchFilter::Remote)));
        ...
        .await;
        fs.set_head_for_repo(
            path!("/dir/.fslckout").as_ref(),
            &[("file.txt", "head_text".to_string())],
            "deadbeef",
        );
        ...
        let repository = cx
            ...
            .unwrap();
        assert!(cx.read(|cx| repository.read(cx).kind().is_fossil()));

        let branches = vec![create_test_branch("trunk", true, None, Some(1000))];
        let (branch_list, mut cx) = init_branch_list_test(Some(repository), branches, cx).await;

        branch_list.update(&mut cx, |branch_list, cx| {
            let picker = branch_list.picker.read(cx);
            assert!(!supports_branch_filter(picker.delegate.repo.as_ref(), cx));
            assert!(picker.delegate.branch_filter == BranchFilter::All);
        });

        branch_list.update_in(&mut cx, |branch_list, window, cx| {
            branch_list.set_branch_filter(BranchFilter::Remote, window, cx);
        });
        branch_list.update(&mut cx, |branch_list, cx| {
            assert!(branch_list.picker.read(cx).delegate.branch_filter == BranchFilter::All);
            assert!(
                cx.try_global::<GlobalBranchFilter>()
                    .is_some_and(|filter| filter.0 == BranchFilter::Remote)
            );
        });
    }
