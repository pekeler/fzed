...
use git::repository::{RepositoryKind, Worktree as GitWorktree};
use gpui::{
...
fn remove_worktree_command(repository_kind: RepositoryKind, path: &Path, force: bool) -> String {
    if repository_kind.is_fossil() {
        if force {
            "fossil close --force".to_string()
        } else {
            "fossil close".to_string()
        }
    } else if force {
...
struct WorktreeRemoveForceDeletePrompt {
    ...
    message: fn(RepositoryKind, &str) -> String,
}
...
fn dirty_worktree_force_delete_prompt(
    repository_kind: RepositoryKind,
    display_name: &str,
) -> String {
    if repository_kind.is_fossil() {
        format!("Checkout \"{display_name}\" contains modified or untracked files. Force close it?")
    } else {
        format!(
...
fn force_delete_prompt_for_worktree_remove_error(
    ...
    WORKTREE_REMOVE_FORCE_DELETE_PROMPTS
        ...
        .map(|prompt| (prompt.message)(repository_kind, display_name))
}
...
impl Render for DeleteWorktreeTooltip {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let repository_kind = self
            .picker
            .read_with(cx, |picker, _| picker.delegate.repository_kind)
            .unwrap_or_default();

        if force_delete {
            Tooltip::for_action_in(
                if repository_kind.is_fossil() {
                    "Force Close Checkout"
                } else {
...
impl WorktreePickerDelegate {
    ...
    fn delete_worktree(
        ...
        let repository_kind = self.repository_kind;
            ...
            if let Err(error) = result {
                if let Some(workspace) = workspace.upgrade() {
                    cx.update(|_window, cx| {
                        show_error_toast(
                            workspace,
                            remove_worktree_command(repository_kind, &path, attempted_force),
                            error,
                            cx,
                        )
                    })?;
