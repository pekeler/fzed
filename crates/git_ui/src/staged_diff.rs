...
impl StagedDiff {
    ...
    pub fn deploy_at(
        ...
        if intended_repo
            .as_ref()
            .is_some_and(|repository| !repository.read(cx).kind().supports_hunk_staging())
        {
            return;
        }
        telemetry::event!(
    ...
    pub(crate) fn new(
        ...
        let diff = cx.new(|cx| {
            DiffMultibuffer::new(
                ...
                Capability::ReadOnly,
                "No staged changes",
                move |editor, _buffer_rename_sources, cx| {
        ...
        self.workspace
            .read_with(cx, |workspace, cx| {
                if let Some(git_panel) = workspace.panel::<GitPanel>(cx) {
                    unstage_all = git_panel.read(cx).can_unstage_all(cx);
                }
            })
            .ok();
