...
use git::{
    ...
    StageAll, StashAll, StashApply, StashPop, StashStaged, StashTracked, ToggleFillCommitEditor,
...
pub use zed_actions::source_control_panel::ToggleFocus;
use zed_actions::{
...
fn group_by_for_repository(
    repository_kind: RepositoryKind,
    group_by: GitPanelGroupBy,
) -> GitPanelGroupBy {
    if repository_kind.is_fossil() && group_by == GitPanelGroupBy::Staging {
...
/// Prompts for an optional stash name. Confirming with an empty editor lets the
/// repository generate its default description.
...
fn git_panel_context_menu(
    ...
    ContextMenu::build(window, cx, |context_menu, _, _| {
        context_menu
            ...
            .when(
                !state.is_fossil && group_by == GitPanelGroupBy::Status,
                |context_menu| {
                    context_menu.action_disabled_when(
                        !state.can_stash || !state.has_tracked_changes,
                        "Stash Tracked",
            ...
            .when(
                !state.is_fossil && group_by == GitPanelGroupBy::Staging,
                |context_menu| {
                    context_menu.action_disabled_when(
                        !state.can_stash || !state.has_staged_changes,
                        "Stash Staged",
...
fn git_panel_view_options_menu(
    ...
    let view_options_menu_state = Rc::new(Cell::new(GitPanelViewOptionsMenuState {
        ...
        group_by: group_by_for_repository(
            repository_kind,
...
struct CommitHistoryEntry {
    ...
    fossil_labels: Vec<(SharedString, bool)>,
}
    ...
    fn from(commit: &Arc<InitialGraphCommitData>) -> Self {
        Self {
            ...
            tag_names: commit
                ...
                .collect(),
            fossil_labels: history_labels(commit, RepositoryKind::Fossil)
                .into_iter()
                .map(|(name, is_branch)| (SharedString::from(name.to_string()), is_branch))
                .collect(),
        }
    }
}
...
impl GitPanel {
                ...
                this.update(cx, |this, cx| {
                    let action = if repository_kind.is_fossil() {
                        RemoteAction::FossilSync(match fetch_options {
                            ...
                            FetchOptions::Remote(remote) => Some(remote.name),
                        })
                    } else {
                        match fetch_options {
    ...
    fn update_visible_entries(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let settings = GitPanelSettings::get_global(cx);
        let sort_by = settings.sort_by;
        let group_by = group_by_for_repository(self.active_repository_kind(cx), settings.group_by);
        ...
        for status_entry in repo.cached_status() {
            ...
            let staging = if repo.kind().is_fossil() {
                if repo.fossil_path_included_for_check_in(&status_entry.repo_path) {
                    StageStatus::Staged
                } else {
                    StageStatus::Unstaged
                }
            } else {
                status_entry.status.staging()
            };
                ...
                self.single_staged_entry =
                    repo.status_for_path(&ops.repo_path)
                        .map(|status| GitStatusEntry {
                            ...
                            rename_source: status.rename_source,
                        });
            }
        }
    ...
    pub fn configure_commit_button(&self, cx: &mut Context<Self>) -> (bool, &'static str) {
        ...
        } else if kind.is_fossil() && self.conflicted_count > 0 {
    ...
    pub(crate) fn toggle_fill_commit_editor(
        &mut self,
    ...
    fn render_git_changes_actions_menu(
        ...
        let state = GitMenuState {
            can_restore: has_write_access && self.can_restore_active_repository(cx),
            can_stash: has_write_access && self.can_stash_active_repository(cx),
    ...
    fn render_git_changes_actions_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let repository_kind = self.active_repository_kind(cx);
        let stage = self.primary_changes_action_stages();
        let (text, action, tooltip) = match (repository_kind.is_fossil(), stage) {
            (true, true) => (
                "Include All",
                StageAll.boxed_clone(),
                "fossil commit FILE...",
            ...
            (false, true) => ("Stage All", StageAll.boxed_clone(), "git add --all"),
            (false, false) => ("Unstage All", UnstageAll.boxed_clone(), "git reset"),
        };
                ...
                .tooltip({
                    ...
                    move |_window, cx| {
                        if repository_kind.is_fossil() {
                            ...
                            Tooltip::for_action_in(tooltip, action.as_ref(), &focus_handle, cx)
                        }
                    }
                })
        ...
        v_flex().flex_1().size_full().overflow_hidden().map(|this| {
            ...
            match &self.commit_history {
                ...
                CommitHistory::Loading => this.child(Self::render_history_placeholder(
                    history_loading_label(repository_kind),
                )),
                CommitHistory::Loaded(entries) if entries.is_empty() => this.child(
                    Self::render_history_placeholder(if repository_kind.is_fossil() {
                        "No check-ins yet"
                    } else {
                        "No commits yet"
                    }),
                ...
                CommitHistory::Loaded(_) => match self.render_commit_history(window, cx) {
                    ...
                    None => this.child(Self::render_history_placeholder(
                        if repository_kind.is_fossil() {
                        ...
                        move |range, window, cx| {
                            ...
                            entries[range.clone()]
                                ...
                                .map(|(ix, (entry, data))| {
                                    ...
                                    let fossil_labels = entry.fossil_labels.clone();
                                    ...
                                    v_flex()
                                        ...
                                        .child(
                                            h_flex()
                                                ...
                                                .when(repository_kind.is_fossil(), |this| {
                                                    this.children(fossil_labels.into_iter().map(
                                                        |(name, is_branch)| {
                                                            let accent_color = cx
                                                                .theme()
                                                                .accents()
                                        ...
                                        .when(!has_context_menu, |this| {
                                            this.tooltip(move |_, cx| {
                                                ...
                                                Tooltip::with_meta(
                                                    history_entry_tooltip_title(repository_kind),
                                                    None,
                                                    description,
                                                    cx,
                                                )
    ...
    fn deploy_entry_context_menu(
        ...
        let should_stage = stage_intent.resolve_with(|| match repo {
    ...
    fn deploy_panel_context_menu(
        ...
        let context_menu = git_panel_context_menu(
            GitMenuState {
                can_restore: has_write_access && self.can_restore_active_repository(cx),
                can_stash: has_write_access && self.can_stash_active_repository(cx),
                can_select_files: has_write_access
                        ...
                        Checkbox::new(checkbox_id, toggle_state)
                            ...
                            .tooltip(move |_window, cx| {
                                if resolved_conflict {
                                    ...
                                    let action = if stage_intent.resolve_with(|| stage_status) {
                                        "Include"
                                    } else {
                                        "Exclude"
                                    };
                        ...
                        Checkbox::new(checkbox_id, toggle_state)
                            ...
                            .tooltip(move |_window, cx| {
                                if resolved_conflict {
                                    ...
                                    let action = if repository_kind.is_fossil() {
                                        if stage_intent.resolve_with(|| stage_status) {
                                            "Include"
                                        } else {
                                            "Exclude"
                                        }
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .when(has_mutation_access && !project.is_read_only(cx), |this| {
                this.on_action(cx.listener(GitPanel::toggle_skip_hooks))
...
mod tests {
    ...
    use gpui::{Action, TestAppContext, UpdateGlobal, VisualTestContext, point, px};
    use indoc::indoc;
    use project::FakeFs;
    ...
    #[gpui::test]
    async fn test_fossil_view_file_tracked(cx: &mut TestAppContext) {
        init_test(cx);
        ...
        let entry_index = panel
            ...
            .expect("tracked Fossil file should exist in the changes list");
    ...
    fn test_fossil_menu_labels_and_stash_policy() {
        ...
        assert_eq!(
            group_by_for_repository(RepositoryKind::Fossil, GitPanelGroupBy::Staging),
            GitPanelGroupBy::None
        );
        assert_eq!(
            group_by_for_repository(RepositoryKind::Fossil, GitPanelGroupBy::Status),
            GitPanelGroupBy::Status
        );
        assert_eq!(
            group_by_for_repository(RepositoryKind::Git, GitPanelGroupBy::Staging),
    ...
    async fn test_fossil_panel_file_selection_and_check_in_state(cx: &mut TestAppContext) {
        ...
        panel.update_in(cx, |panel, window, cx| {
            panel.toggle_staged_for_entry(&tracked_entry, StageIntent::Toggle, window, cx);
        });
