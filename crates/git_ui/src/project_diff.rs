...
impl ProjectDiff {
    ...
    fn deploy(
        ...
        let use_default_branch = project.read(cx).active_repository(cx).is_some_and(|repo| {
            git_store.read(cx).diff_base_for_repo(&repo, cx) == GitDiffBaseSetting::DefaultBranch
        });
    ...
    fn new_impl(
        ...
        let diff = cx.new(|cx| {
            DiffMultibuffer::new(
                ...
                move |editor, buffer_rename_sources, cx| {
                    ...
                    editor.rhs_editor().update(cx, |rhs_editor, _cx| {
                        ...
                        rhs_editor.register_addon(GitPanelAddon {
                            ...
                            buffer_rename_sources: Some(buffer_rename_sources),
                        });
                    });
                },
                project.clone(),
    ...
    fn button_states(&self, cx: &App) -> ButtonStates {
        ...
        let mut repository_kind = RepositoryKind::default();
        self.workspace
            ...
            .when(
                ...
                |this| {
                    this.child(
                        ...
                        .when(button_states.repository_kind.is_fossil(), |button| {
                            button
                                .tooltip(Tooltip::text("Exclude all files from the next check-in"))
                        })
                        .when(!button_states.repository_kind.is_fossil(), |button| {
                            button.tooltip(Tooltip::for_action_title_in(
                                "Unstage All Changes",
                                &UnstageAll,
                                &focus_handle,
                            ))
            ...
            .when(
                ...
                |this| {
                    this.child(
                        ...
                        .when(button_states.repository_kind.is_fossil(), |button| {
                            button.tooltip(Tooltip::text("Include all files in the next check-in"))
                        })
                        .when(!button_states.repository_kind.is_fossil(), |button| {
                            button.tooltip(Tooltip::for_action_title_in(
                                "Stage All Changes",
...
mod tests {
    ...
    #[gpui::test]
    async fn test_diff_action_uses_head_for_fossil(cx: &mut TestAppContext) {
        init_test(cx);
        ...
        fs.insert_tree(
            ...
            json!({
                ".fslckout": {},
                "changed.txt": "working copy\n",
            }),
        )
        .await;
        fs.set_head_and_index_for_repo(
            Path::new(path!("/project/.fslckout")),
            &[("changed.txt", "checkout\n".into())],
        );
        ...
        let repository_id = project.read_with(cx, |project, cx| {
            ...
            assert_eq!(repository.read(cx).kind(), RepositoryKind::Fossil);
            assert_eq!(
                project
                    .git_store()
                    .read(cx)
                    .diff_base_for_repo(&repository, cx),
                GitDiffBaseSetting::Head
            );
        ...
        workspace.update(cx, |workspace, cx| {
            assert!(workspace.active_item_as::<ProjectDiff>(cx).is_some());
            assert_eq!(workspace.items_of_type::<BranchDiff>(cx).count(), 0);
        });
        project.read_with(cx, |project, cx| {
            assert!(
                project
