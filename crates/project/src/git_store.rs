...
use git::{
    ...
    repository::{
        ...
        FileHistoryChangedFileSets, FossilSyncState, GitCommitTemplate, GitRepository,
...
pub struct Repository {
    ...
    fossil_included_paths: HashSet<RepoPath>,
    job_id: JobId,
...
impl GitStore {
    ...
    pub fn open_display_diff(
        ...
        if let Some((repo, repo_path)) =
            self.repository_and_path_for_buffer_id(buffer.read(cx).remote_id(), cx)
            && self.diff_base_for_repo(&repo, cx) == GitDiffBaseSetting::DefaultBranch
    ...
    async fn handle_update_repository(
        ...
        this.update(&mut cx, |this, cx| {
            ...
            let common_dir_abs_path: Option<Arc<Path>> = update
                ...
                .as_deref()
                .map(|p| Path::new(p).into());
            let kind = repository_kind_from_proto(update.repository_kind);
    ...
    async fn handle_commit(
        ...
        repository_handle
            .update(&mut cx, |repository_handle, cx| {
                ...
                if paths.is_empty() {
                    repository_handle.commit(message, name.zip(email), options, askpass, cx)
                } else {
                    repository_handle.commit_paths(
                        message,
                        name.zip(email),
                        options,
                        askpass,
    ...
    pub fn diff_base_for_repo(&self, repo: &Entity<Repository>, cx: &App) -> GitDiffBaseSetting {
        match repo.read(cx).kind() {
            RepositoryKind::Git => self.diff_base,
            RepositoryKind::Fossil => GitDiffBaseSetting::Head,
        }
    }
    ...
    fn display_status(
        ...
        if self.diff_base_for_repo(repo, cx) == GitDiffBaseSetting::DefaultBranch
            && let Some((buffers, _)) = self
                .display_diffs
                .get(&repo.read(cx).id)
                .and_then(|state| state.buffers.as_ref())
    ...
    fn refresh_diff_base_for_repo(&mut self, repo_id: RepositoryId, cx: &mut Context<Self>) {
        ...
        if self.diff_base_for_repo(&repo, cx) == GitDiffBaseSetting::Head {
            self.display_diffs.remove(&repo_id);
            return;
        }
...
fn repository_kind_for_metadata_path(metadata_path: &Path) -> RepositoryKind {
    if metadata_path
        .file_name()
...
impl RepositorySnapshot {
    ...
    fn initial_update(&self, project_id: u64) -> proto::UpdateRepository {
        proto::UpdateRepository {
            ...
            repository_kind: Some(repository_kind_to_proto(self.kind)),
...
impl Repository {
    ...
    fn prune_fossil_included_paths(&mut self) -> bool {
        ...
        let statuses_by_path = &self.snapshot.statuses_by_path;
        let len_before = self.fossil_included_paths.len();
        self.fossil_included_paths.retain(|path| {
            statuses_by_path
                .get(&PathKey(path.as_ref().clone()), ())
                .is_some()
        });
    ...
    fn local(
        ...
        let kind = repository_kind_for_metadata_path(&dot_git_abs_path);
                    ...
                    .background_spawn(async move {
                        ...
                        ) in &repo_diff_state_updates
                        {
                            ...
                            if current_head_text.is_some() && !*is_symlink {
                                let head_path = head_text_repo_path.as_ref().unwrap_or(repo_path);
                                revisions.push(format!("HEAD:{}", head_path.as_unix_str()));
                            }
                        }
                        ...
                        for (buffer, _, _, is_symlink, current_index_text, current_head_text) in
    ...
    fn stage_or_unstage_entries(
        ...
        if self.snapshot.kind.is_fossil() {
            ...
            for entry in entries {
                if stage {
                    changed |= self.fossil_included_paths.insert(entry);
                } else {
                    changed |= self.fossil_included_paths.remove(&entry);
                }
            }
            if changed {
                self.sync_fossil_included_paths_snapshot();
    ...
    pub fn stage_all(&mut self, cx: &mut Context<Self>) -> Task<anyhow::Result<()>> {
        ...
        let fossil_included_paths = self.fossil_included_paths.clone();
        if snapshot.kind.is_fossil() {
            let to_stage = cx.background_spawn(async move {
                snapshot
                    .status()
                    .filter_map(|entry| {
                        if fossil_included_paths.contains(&entry.repo_path) {
                            None
                        } else {
    ...
    pub fn commit(
        ...
        let fossil_commit_paths = self.snapshot.kind.is_fossil().then(|| {
            self.fossil_included_paths
                .iter()
                .cloned()
                .collect::<Vec<_>>()
        });
        self.send_commit_job(
            message,
            name_and_email,
            options,
            askpass,
    ...
    pub fn commit_paths(
        ...
        self.send_commit_job(message, name_and_email, options, askpass, Some(paths))
    }
    ...
    fn send_commit_job(
        ...
        options: CommitOptions,
        askpass: AskPassDelegate,
        fossil_commit_paths: Option<Vec<RepoPath>>,
    ) -> oneshot::Receiver<Result<()>> {
        let id = self.id;
        ...
        self.send_job(
            "commit",
            Some(
                if fossil_commit_paths.is_some() {
                    "fossil commit"
                } else {
                    "git commit"
                }
    ...
    pub fn fetch(
        ...
        let id = self.id;
        let repository_kind = self.snapshot.kind;
        let status = if repository_kind.is_fossil() {
            "fossil sync"
        } else {
            "git fetch"
        };
            ...
            move |git_repo, mut cx| async move {
                match git_repo {
                    RepositoryState::Local(LocalRepositoryState {
                        ...
                        if result.is_ok() {
                            if repository_kind.is_fossil() {
                                refresh_fossil_snapshot_after_command(this, backend, &mut cx)
                                    .await?;
                            } else {
                                Self::refresh_branch_list(&this, backend, updates_tx, &mut cx)
                                    .await?;
                            }
                        }
                        result
    ...
    pub(crate) fn apply_remote_update(
        ...
        self.snapshot.kind = repository_kind_from_proto(update.repository_kind);
    ...
    fn load_committed_text_for_paths(
        ...
        let rx = self.send_job("load_committed_text", None, move |state, _| async move {
            match state {
                RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
                    let head_path = head_path.as_ref().unwrap_or(&repo_path);
                    let revisions = vec![
                        format!("HEAD:{}", head_path.as_unix_str()),
        ...
        let result = self.send_keyed_job(
            ...
            |state, mut cx| async move {
                ...
                this.update(&mut cx, |this, cx| {
                    ...
                    let statuses_changed = !changed_path_statuses.is_empty();
                    if statuses_changed {
                        this.snapshot
                            .statuses_by_path
                    ...
                    let fossil_selection_changed = this.prune_fossil_included_paths();
                    if statuses_changed || fossil_selection_changed {
                        cx.emit(RepositoryEvent::StatusesChanged);
                    }
...
fn repository_kind_to_proto(kind: RepositoryKind) -> i32 {
    match kind {
        RepositoryKind::Git => proto::RepositoryKind::Git as i32,
        RepositoryKind::Fossil => proto::RepositoryKind::Fossil as i32,
    }
}

fn repository_kind_from_proto(kind: Option<i32>) -> RepositoryKind {
    match proto::RepositoryKind::from_i32(kind.unwrap_or(proto::RepositoryKind::Git as i32)) {
        Some(proto::RepositoryKind::Fossil) => RepositoryKind::Fossil,
        Some(proto::RepositoryKind::Git) | None => RepositoryKind::Git,
    }
}
...
async fn compute_snapshot(
    ...
    let fossil_sync_state = backend.fossil_sync_state().await.log_err().flatten();
    ...
    this.update(cx, |this, cx| {
        let statuses_changed = statuses_by_path != this.snapshot.statuses_by_path;
