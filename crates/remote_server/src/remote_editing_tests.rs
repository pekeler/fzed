...
#[gpui::test]
async fn test_add_path_to_fossil_ignore_in_remote_repository(
    cx: &mut TestAppContext,
    server_cx: &mut TestAppContext,
) {
    let fs = FakeFs::new(server_cx.executor());
    ...
    .await;
    fs.set_branch_name(Path::new("/project/.fslckout"), Some("trunk"));
    fs.set_head_for_repo(
        Path::new("/project/.fslckout"),
        &[(".fossil-settings/ignore-glob", "existing\n".into())],
        "head-sha",
    );
    ...
    for (path, is_dir) in [("tmp.txt", false), ("logs", true), ("tmp.txt", false)] {
        ...
        cx.update(|cx| {
            repository.update(cx, |repository, _| {
                repository.add_path_to_repository_ignore(&repo_path, is_dir)
            })
        })
        .await
        .expect("add to Fossil ignore-glob request should complete")
        .expect("add to Fossil ignore-glob should succeed for remote repository");
    }
    ...
    assert_eq!(
        fs.load(Path::new("/project/.fossil-settings/ignore-glob"))
            .await
            .expect("Fossil ignore-glob should be readable"),
        "existing\ntmp.txt\nlogs/\n"
    );
}
