...
actions!(
    cli,
    [
        /// Installs the FZed CLI tool to the system PATH.
        InstallCliBinary,
    ]
);

const CANT_INSTALL_DOCS_URL: &str = "https://github.com/pekeler/fzed#command-line-interface";
...
async fn install_script(cx: &AsyncApp) -> Result<Option<PathBuf>> {
    ...
    let link_path = Path::new("/usr/local/bin/fzed");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    const LINUX_PROMPT_DETAIL: &str = "If you installed FZed from an official release, add ~/.local/bin to your PATH.\n\nIf you installed FZed from a different source like your package manager, then you may need to create an alias or symlink manually.";
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        let path = match install_script(cx.deref()).await {
            ...
            Ok(None) => return Ok(()),
            Err(error) => {
                log::error!("failed to install fzed CLI: {error:#}");
                        ...
                        |cx| {
                            cx.new(|cx| {
                                MessageNotification::new(
                                    "You can add `fzed` to your PATH manually.",
                                    cx,
                                )
                                .with_title("Couldn't install the FZed CLI")
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            struct InstalledFZedCli;

            workspace.show_toast(
                Toast::new(
                    NotificationId::unique::<InstalledFZedCli>(),
                    format!(
                        "Installed `fzed` to {}. You can launch {} from your terminal.",
    ...
    .detach_and_prompt_err("Cannot install the FZed CLI", window, cx, |_, _, _| None);
}
