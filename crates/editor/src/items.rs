...
///
/// In the modifications modes, only lines with uncommitted changes are formatted.
/// When no source control diff is available for a buffer, `modifications` skips formatting while `modifications_if_available`
...
fn compute_format_target(
    ...
    for buffer_entity in buffers.iter() {
        ...
        let buffer_id = buffer.remote_id();
        let is_fossil = git_store
            .repository_and_path_for_buffer_id(buffer_id, cx)
            .is_some_and(|(repository, _)| repository.read(cx).kind().is_fossil());
        let diff = if is_fossil {
            git_store.get_uncommitted_diff(buffer_id, cx)
        } else {
            git_store.get_unstaged_diff(buffer_id, cx)
        };
        let Some(diff_snapshot) = diff.map(|diff| diff.read(cx).snapshot(cx)) else {
