...
impl Editor {
    ...
    pub fn apply_toggle(
        ...
        hunks.retain(|hunks| self.buffer_supports_hunk_staging(hunks.buffer_id, cx));
        if hunks.is_empty() {
            return;
        }
    ...
    pub fn apply_restore(
        ...
        let mut hunks = self.resolve_diff_hunks(hunks, cx);
        hunks.retain(|hunks| self.buffer_supports_hunk_restore(hunks.buffer_id, cx));
        if hunks.is_empty() {
            return;
        }
        let delegate = self.diff_hunk_delegate();
...
fn editor_hunk_control_support(
    ...
    let editor = editor.read(cx);
    (
        editor.buffer_supports_hunk_staging(buffer_id, cx),
        editor.buffer_supports_hunk_restore(buffer_id, cx),
    )
}
