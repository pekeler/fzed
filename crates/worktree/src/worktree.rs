...
use fuzzy::CharBag;
use git::{
    BISECT_LOG, COMMIT_MESSAGE, DOT_FOSSIL, DOT_GIT, FETCH_HEAD, FOSSIL_CHECKOUT,
    FOSSIL_IGNORE_GLOB, FOSSIL_SETTINGS_DIR, FSMONITOR_DAEMON, GC_PID, GITIGNORE, HOOKS_DIR,
...
impl LocalSnapshot {
    ...
    async fn ignore_stack_for_abs_path(
        ...
        for (index, ancestor) in abs_path.ancestors().enumerate() {
            ...
            // A nested repository's files are still governed by the repository-specific
            // ignore rules of the outer repository that contains it.
            if let Some((repo_exclude, _)) = self.repo_exclude_by_work_dir_abs_path.get(ancestor) {
                repo_excludes.push(repo_exclude.clone());
            }

            let is_repo_root = repository_metadata_path_for_directory(ancestor, fs)
                .await
                .is_some();
            if is_repo_root {
...
async fn build_repository_exclude(
    ...
    if is_fossil_repository_metadata_path(dot_git_abs_path) {
        ...
        build_gitignore_with_root(&exclude_abs_path, work_dir_abs_path, fs).await
    }
}
    ...
    async fn process_events(&self, mut events: Vec<PathEvent>) {
...

        {
            ...
            for (ix, event) in events.iter().enumerate() {
                ...
                // rescanned path were dropped, possibly including repository metadata
                // changes. Reload every repository whose metadata is under the rescanned
                // path, since those changes may have gone unseen.
