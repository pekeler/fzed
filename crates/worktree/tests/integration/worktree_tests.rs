...
#[gpui::test]
async fn test_outer_fossil_ignore_glob_applies_within_nested_repo(
    ...
    worktree.update(cx, |worktree, _cx| {
        check_worktree_entries(
            worktree,
            WorktreeExpectations {
                ignored_paths: &["nested/ignored.generated"],
                tracked_paths: &["nested/kept.txt"],
                ..Default::default()
            },
        );
    });
...
#[gpui::test]
async fn test_watcher_overflow_rescan_reloads_fossil_state(cx: &mut TestAppContext) {
    init_test(cx);
    let fs = FakeFs::new(cx.background_executor.clone());
    fs.insert_tree(
        path!("/root"),
    ...
    let repository_update_count: Rc<Cell<usize>> = Rc::new(Cell::new(0));
    tree.update(cx, {
        let repository_update_count = repository_update_count.clone();
        |_, cx| {
            cx.subscribe(&cx.entity(), move |_, _, event, _| {
                if matches!(event, Event::UpdatedGitRepositories(_)) {
                    repository_update_count.set(repository_update_count.get() + 1);
                }
            })
            .detach();
        }
    ...
    assert!(
        repository_update_count.get() > 0,
        "a watcher overflow rescan should reload Fossil state"
    );
}
